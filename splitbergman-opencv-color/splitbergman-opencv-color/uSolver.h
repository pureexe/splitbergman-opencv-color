...
Mat uSolver(Mat u, Mat w, Mat original_image, Mat b, Mat lambda,double theta, double omega,int depth,int max_gauss_seidel) {
	int height = u.rows;
	int width = u.cols;
	int i, j, k;
	Mat divergence = Divergence(w - b, depth);
	for (k = 0; k < max_gauss_seidel; k++) {
		//top - left corner
		u.at<double>(0, 0) = uGaussKernel(
			u.at<double>(0, 0), original_image.at<double>(0, 0),
			lambda.at<double>(0, 0), theta, divergence.at<double>(0, 0), omega, depth,
		...
		for (i = 1; i < width - 1; i++) {
			u.at<double>(0, i) = uGaussKernel(u.at<double>(0, i), original_image.at<double>(0, i),
				lambda.at<double>(0, i), theta, divergence.at<double>(0, i), omega, depth,
		...
		u.at<double>(0, width - 1) = uGaussKernel(u.at<double>(0, width - 1), original_image.at<double>(0, width - 1),
			lambda.at<double>(0, width - 1), theta, divergence.at<double>(0, width - 1), omega, depth,
		...
		for (i = 1; i < height - 1; i++) {
			...
			u.at<double>(i, 0) = uGaussKernel(u.at<double>(i, 0), original_image.at<double>(i, 0),
				lambda.at<double>(i, 0), theta, divergence.at<double>(i, 0), omega, depth,
			...
			for (j = 1; j < width - 1; j++) {
				u.at<double>(i, j) = uGaussKernel(u.at<double>(i, j), original_image.at<double>(i, j),
					lambda.at<double>(i, j), theta, divergence.at<double>(i, j), omega, depth,
			...
			u.at<double>(i, width - 1) = uGaussKernel(u.at<double>(i, width - 1), original_image.at<double>(i, width - 1),
				lambda.at<double>(i, width - 1), theta, divergence.at<double>(i, width - 1), omega, depth,
		...
		u.at<double>(height - 1, 0) = uGaussKernel(u.at<double>(height - 1, 0), original_image.at<double>(height - 1, 0),
			lambda.at<double>(height - 1, 0), theta, divergence.at<double>(height - 1, 0), omega, depth,
		...
		for (i = 1; i < width - 1; i++) {
			u.at<double>(height - 1, i) = uGaussKernel(u.at<double>(height - 1, i), original_image.at<double>(height - 1, i),
				lambda.at<double>(height - 1, i), theta, divergence.at<double>(height - 1, i), omega, depth,
		...
		u.at<double>(height - 1, width - 1) = uGaussKernel(u.at<double>(height - 1, width - 1), original_image.at<double>(height - 1, width - 1),
			lambda.at<double>(height - 1, width - 1), theta, divergence.at<double>(height - 1, width - 1), omega, depth,
